...
const KPICard = ({ title, value, subValue, trend, trendValue, color = 'blue', prefix = '', suffix = '', decimals = 2 }: any) => {
    ...
    <div>
      <h3 className="text-[11px] font-bold text-text-secondary uppercase tracking-wider mb-2">{title}</h3>
        <span className="font-mono font-bold text-text-primary text-2xl">
          <span>{safeScalar(prefix)}</span>
          <span>{safeNumber(value).toLocaleString(undefined, { minimumFractionDigits: decimals, maximumFractionDigits: decimals })}</span>
          <span>{safeScalar(suffix)}</span>
        </span>
    </div>

      <div className="mt-3 flex items-center justify-between">
        <span className="text-xs text-text-tertiary">{safeScalar(subValue)}</span>
        {trendValue && (
          <span className={`text-xs font-bold ${trend === 'up' ? 'text-accent-success' : 'text-accent-danger'}`}>
            {trend === 'up' ? '▲' : '▼'} {safeScalar(trendValue)}
          </span>
        )}
      </div>
    </div>
