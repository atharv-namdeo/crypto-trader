...
def compute_adx(df: pd.DataFrame, period: int = 14) -> pd.Series:
    ...
    di_plus   = 100 * dm_plus.rolling(period).mean() / (atr14 + 1e-9)
    di_minus  = 100 * dm_minus.rolling(period).mean() / (atr14 + 1e-9)
...
def compute_vwap(df: pd.DataFrame) -> float:
    ...
    return (cumulative_tp_vol / (cumulative_vol + 1e-9)).iloc[-1]
